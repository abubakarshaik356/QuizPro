...
<%
    // -----------------------------
    // Backend-defined variables
    // -----------------------------
    String quizTitle = (String) request.getAttribute("quizTitle");
    String quizCategory = (String) request.getAttribute("quizCategory");

    List<String> questions = (List<String>) request.getAttribute("questions");
    if (questions == null) {
        questions = new java.util.ArrayList<>();
        questions.add("Which Python structure is mutable?");
        questions.add("Big O of binary search?");
        questions.add("FIFO structure is called...");
    }
    ...
    <style>
       :root {
        ...
        .sidebar .logo {
            ...
            color: white; text-decoration: none; display: block; margin-bottom: 30px;
        }
        .sidebar .logo span { color: var(--accent-color); }

        .sidebar nav a {
            display: block; padding: 15px 10px; margin-bottom: 5px;
            border-radius: 8px; color: rgba(255,255,255,0.9);
            text-decoration: none; font-weight: 600;
        }
        .sidebar nav a.active,
        .sidebar nav a:hover { background-color: rgba(255,255,255,0.15); }

        /* Main content */
        .main-content { flex-grow: 1; padding: 30px; }

        .page-header { border-bottom: 1px solid #ddd; margin-bottom: 20px; }
        ...
        .question-form-card {
            ...
            border-radius: 10px; box-shadow: 0 4px 10px var(--shadow-light);
        }
        ...
        .add-option-btn { color: var(--success-color); }
        .remove-option-btn { color: var(--danger-color); }
        ...
        .question-list-card ul { list-style: none; padding: 0; }

        .question-list li {
            padding: 10px; border-bottom: 1px dashed #ddd;
            cursor: pointer;
        }
        .question-list li:hover { background: #f5f5f5; }

        .finish-quiz-btn {
            display: block; margin-top: 20px; text-align: center;
            padding: 12px; background: var(--accent-color);
        ...
        <div class="editor-container">
            ...
            <div class="question-list-card">
                ...
                <ul class="question-list">
                    <% int i=1; for(String q : questions){ %>
                        <li><%= i %>. <%= q %></li>
                    <% i++; } %>
                </ul>
...
<script>
/* ---------------------------
   OPTION GENERATOR LOGIC (YOUR ORIGINAL)
----------------------------*/
...
function updateOptions() {
    ...
    if (type === "true-false") {
        document.getElementById("addOptionUIBtn").style.display = "none";
    } else {
        document.getElementById("addOptionUIBtn").style.display = "inline-block";
    }
    ...
    for (let i = 1; i <= optionCount; i++) {
        ...
        div.innerHTML = `
            <input type="${inputType}" name="correct_answer" value="${i}">
            <input type="text" name="option${i}" value="${label}" ${type === "true-false" ? "readonly" : ""} required>
        `;
...
function addOption() {
    ...
    div.innerHTML = `
        <input type="${inputType}" name="correct_answer" value="${dynamicOptionCount}">
        <input type="text" name="option${dynamicOptionCount}" placeholder="Option ${dynamicOptionCount}" required>
        <button type="button" class="remove-option-btn" onclick="this.parentElement.remove()"><i class="fas fa-times-circle"></i></button>
    `;
