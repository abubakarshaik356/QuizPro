...
<body>
			...
			<div class="test-status-area">
				<div class="status-box">
					<i class="fas fa-list-ol"></i> Question: <span
						style="color: var(--primary-color);"><%= qIndex + 1 %></span> / 50
				</div>
				<div class="status-box">
					<i class="fas fa-clock"></i> Time Remaining: <span class="timer">25:30</span>
				</div>
				<button class="end-test-button" onclick="confirmEndTest()">End
					Test</button>
			</div>
		</div>
	</header>
	...
	<div class="container test-grid">
		...
		<div class="question-column">
			<div class="question-area">
				...
				<h3 class="question-text"><%=q.getQuestion() %></h3>
				...
				<%-- <%=if(<%= %>){ %> --%>
				<%if(q.getOptionType().equals("mcq")) {%>
				<form id="quizForm">
					<label class="answer-option"> <input type="radio"
						name="answer_q5" value="A" class="answer-input"> <span
						class="answer-label"><%=q.getOption1() %>></span>
					</label> <label class="answer-option"> <input type="radio"
						name="answer_q5" value="B" class="answer-input"> <span
						class="answer-label"><%= q.getOption2() %>></span>
					</label> <label class="answer-option"> <input type="radio"
						name="answer_q5" value="C" class="answer-input"> <span
						class="answer-label"><%=q.getOption3() %>></span>
					</label> <label class="answer-option"> <input type="radio"
						name="answer_q5" value="D" class="answer-input"> <span
						class="answer-label"><%= q.getOption4() %>></span>
					</label>
				</form>
				<%} else if(q.getOptionType().equals("multi-select")) {%>
				<form id="quizForm">
					<label class="answer-option"> <input type="checkbox"
						name="answer" value="A" class="answer-input"> <span
						class="answer-label"><%= q.getOption1() %></span>
					</label> <label class="answer-option"> <input type="checkbox"
						name="answer" value="B" class="answer-input"> <span
						class="answer-label"><%= q.getOption2() %></span>
					</label> <label class="answer-option"> <input type="checkbox"
						name="answer" value="C" class="answer-input"> <span
						class="answer-label"><%= q.getOption3() %></span>
					</label> <label class="answer-option"> <input type="checkbox"
						name="answer" value="D" class="answer-input"> <span
						class="answer-label"><%= q.getOption4() %></span>
					</label>
				</form>
				<%} else if(q.getOptionType().equals("true-false")) {%>
				<form id="quizForm">
					<label class="answer-option"> <input type="radio"
						name="answer" value="True" class="answer-input"> <span
						class="answer-label">True</span>
					</label> <label class="answer-option"> <input type="radio"
						name="answer" value="False" class="answer-input"> <span
						class="answer-label">False</span>
					</label>
				</form>
				...
				<%-- <div class="test-footer">
					<a href="QuestionNavigation?qIndex=<%= qIndex - 1 %>" class="nav-button prev-button"><i
						class="fas fa-chevron-left"></i> Previous</a>

					<div>
						<a href="QuestionNavigation?qIndex=<%= qIndex + 1 %>" class="nav-button mark-button"
							style="margin-right: 15px;"><i class="fas fa-flag"></i> Mark
							for Review</a> <a href="TestComplete.jsp"
							class="nav-button next-button">Next <i
    ...
    <!-- Previous button -->
    <% if (qIndex > 0) { %>
        <a href="QuestionNavigation?qIndex=<%= qIndex - 1 %>" 
           class="nav-button prev-button">
           <i class="fas fa-chevron-left"></i> Previous
        </a>
    <% } else { %>
        <button class="nav-button prev-button disabled" disabled>
            <i class="fas fa-chevron-left"></i> Previous
        </button>
    <% } %>
    ...
    <div>
        <!-- Mark for Review -->
        <a href="QuestionNavigation?qIndex=<%= qIndex + 1 %>&mark=true"
           class="nav-button mark-button" style="margin-right: 15px;">
        ...
        <% } else { %>
            <a href="FinishTest" class="nav-button next-button">
