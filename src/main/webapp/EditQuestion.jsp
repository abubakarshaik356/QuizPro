...
<%
    ...
    String answerData = ques.getAnswer() != null ? ques.getAnswer() : "";
%>
    ...
    <div class="editor-container">
        ...
        <div class="question-form-card">
			...
			<form action="UpdateQuestionServletPage" method="post">
                ...
                <div class="form-group">
                    <label>Question Text</label>
                    <textarea name="question_text" required><%= ques.getQuestion() != null ? ques.getQuestion() : "" %></textarea>
                </div>
...
<!-- ======================================
     PASS EXISTING VALUES FROM BACKEND
========================================= -->
<script>
    const existingType = "<%= ques.getOptionType() != null ? ques.getOptionType() : "mcq" %>";
    const existingOptions = [
        "<%= ques.getOption1() != null ? ques.getOption1() : "" %>",
        "<%= ques.getOption2() != null ? ques.getOption2() : "" %>",
        "<%= ques.getOption3() != null ? ques.getOption3() : "" %>",
        "<%= ques.getOption4() != null ? ques.getOption4() : "" %>"
    ];

    const existingAnswer = "<%= answerData %>";  // Comma-separated
</script>


<!-- ======================================
     BUILDING DYNAMIC OPTIONS UI
========================================= -->
<script>
...
function updateOptions() {
    ...
    if (existingType) {
        typeDropdown.value = existingType;
    }
    ...
    const answersArray = existingAnswer.split(",").map(a => a.trim());

    for (let i = 1; i <= optionCount; i++) {

        let label = existingOptions[i - 1] || (`Option ${i}`);
        ...
        div.innerHTML = `
            <input type="${inputType}"
                ...
                value="${label}"
                class="correctOption"
                ${ answersArray.includes(label) ? "checked" : "" }>
...
function addOption() {
    ...
    const optionLabel = "Option " + dynamicOptionCount;
